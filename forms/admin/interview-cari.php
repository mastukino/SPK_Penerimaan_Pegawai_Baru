...
$result = mysqli_query($koneksi, "SELECT * FROM interview ORDER BY hasil DESC");
        ...
        <div id="layoutSidenav">
            ...
            <div class="row container">
                        ...
                        <form name="cari" method="post" action="interview-cari.php" class="row">
                          ...
                          <div class="col-1">
                                <a href='interview.php' class="btn red">Kembali<i class="material-icons right"></i></a>
                            </div>
                ...
                <div class="table-responsive" >
                        <table class="table table-bordered " width="100%" cellspacing="0">
                          ...
                          $sql = "SELECT * FROM interview WHERE id LIKE '%$cari%' OR kode_interview LIKE '%$cari%' OR kode_pelamar LIKE '%$cari%' OR nama LIKE '%$cari%' OR email LIKE '%$cari%' OR no_telpon LIKE '%$cari%' OR nama_lowongan LIKE '%$cari%' OR tanggal_interview LIKE '%$cari%' OR status LIKE '%$cari%' ORDER BY tanggal_interview DESC";
                          $query = mysqli_query($conn,$sql);
                           ...
                           if ($query->num_rows > 0) {
                            while($data = mysqli_fetch_array($query)){
                              $test = $data['kode_interview'];

                            
                              echo "<tr>";
                              echo "<td hidden>".$data['id']." </h6></td>";
                              echo "<td> <h6>".$data['kode_interview']." </h6></td>";
                         
                              echo "<td> <h6> 
                                <a href='penilaian-detail.php?kode_pelamar=$data[kode_pelamar]' style='text-decoration: none;'>

                                ".$data['kode_pelamar']."</a></h6></td>";
                              echo "<td> <h6>".$data['nama']." </h6></td>";  
                              echo "<td> <h6>".$data['email']." </h6></td>";
                              echo "<td> <h6>".$data['no_telpon']." </h6></td>";
                              echo "<td> <h6>".$data['nama_lowongan']." </h6></td>";
                               echo "<td> <h6>".$data['tanggal_interview']." </h6></td>";
                                echo "<td> <h6>".$data['status']." 
                                <br>
                                <a data-id='1' class='verifikasi1' href='laporan-tambah.php?id=$data[id]&status=Lamaran Diterima' style='text-decoration: none;'><i class='material-icons' title='Lamaran Diterima'>done_outline</i></a>

                                <a data-id='1' class='verifikasi2' href='laporan-tambah.php?id=$data[id]&status=Lamaran Ditolak'style='text-decoration: none;'><i class='material-icons' title='Lamaran Ditolak'>work_off</i></a>
                              ...
                              echo "<td class='text-center'> <a target='_blank' href='https://api.whatsapp.com/send?phone=$data[no_telpon]&text=Kami%20PT%20Kingslee%20Infinitas%20Teknologi%20mengundang%20sdr%20$data[nama]%20untuk%20mengikuti%20interview%20di%20alamat%20Lytech%20Home%20Center%20Block%20A%20No%206%20pada%20keesokan%20hari%20pada%20pukul%20jam%2012.00%20WIB%0ADiharapkan%20membawa%20CV%20dan%20berkas%20pelengkap.%0AHarap%20Konfirmasi%20:%20apakah%20bisa%20hadir%20atau%20tidak%0AHRD%20PT%20Kingslee%20Infinitas%20Teknologi' style='text-decoration: none;'><i class='iconify' data-icon='mdi-whatsapp'  width='30px' title='whatsapp' >message</i></a>




                                | <a data-id='1' class='hapus' href='interview-delete.php?id=$data[id]' style='text-decoration: none;'><i class='material-icons' title='Hapus $test'>delete</i></a>
                              ...
                              echo "<tr><td colspan='7'><center><h6><b>'$substr'</b> Tidak Ditemukan. Silahkan Periksa Kembali Keyword Anda</h6></center></td></tr>";}
                          }
                      ?>
